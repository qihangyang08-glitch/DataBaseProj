...
public class ClassController {
    ...
    /**
     * 根据班级名称模糊搜索班级
     * GET /api/classes/search-by-name?name=...
     * 任何已认证的用户都可以搜索
     */
    @GetMapping("/search-by-name")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<Page<ClassResponseDto>>> searchClassesByName(
            @RequestParam("name") String name,
            @PageableDefault(size = 20, sort = "createdAt") Pageable pageable) {

        Page<ClassResponseDto> results = classService.searchClassesByName(name, pageable);

        return ResponseEntity.ok(ApiResponse.success(results, "班级搜索成功"));
    }
